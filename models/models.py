...
class import_records(models.Model):
    ...
    def import_file(self):
        table = 'NULL'
        #le developpeur ou celui qui import, va determiner soit le nom de la colonne(pour l'importer) ou "ne pas import"pour ne l'importe pas)
        importable_fields={
            'champs1': 'code', 
            'champs2': 'ne pas import', #champs2 = rubrique(pour l'importer) ou =ne pas import(pour ne l'importe pas) 
            'champs3': 'montant',
            'champs4':'ne pas import',
            'champs5':'ne pas import'
            }
        #liste des colonnes importables
        list_col=[]
                ...
                if vals:
                    if fr == 0:
                        ...
                        for i in range(0,len(All_col)):
                            #si la colonnes i ==line[i] est pas importable passe à i+1
                        ...
                        tuple2=()
                        cnt=0
                        #on va ajouter juste les colonnes importables cad list_col
                        for i in list_col:
                            tuple1=tuple1+(i,)
                            cnt+=1
...
#chech if s is a number 
def check_value(s):
    try:
        #int or float it's okey.
        float(s)
        return True
    except ValueError:
        return False
